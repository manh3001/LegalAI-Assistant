...
async function analyzeVideo(videoUrl) {
    ...
    try {
        ...
        try {
            const keywordPrompt = `Trích xuất 3-5 cụm từ khóa pháp lý chính xác từ đoạn transcript lỗi sau (ví dụ '204' sửa thành '2024'): "${transcript.substring(0, 1000)}"`;
            const refinedKeywords = await getActiveModel(keywordPrompt);
